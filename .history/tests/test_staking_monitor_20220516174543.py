...
def test_deposit():
    ...
    # Act
    deposit_tx_0 = staking_monitor.deposit(
        {"from": get_account(), "value": value})
    deposit_tx_0.wait(1)

    value_1 = Web3.toWei(0.03, "ether")
    deposit_tx_1 = staking_monitor.deposit(
        {"from": get_account(1), "value": value_1})
    deposit_tx_1.wait(1)

    # check that the balance has increased by the amount of the deposit
    assert staking_monitor.s_userInfos(get_account().address)[
    ...
    # check that the balance has increased by the amount of the deposit
    assert (
        staking_monitor.s_userInfos(get_account(1).address)[
            "depositBalance"] == value_1
    )
    # check if address is added to watchlist
...
def test_set_price_limit_if_user_has_not_deposited_reverts():
    ...
    with pytest.raises(exceptions.VirtualMachineError):
        price_limit_tx = staking_monitor.setPriceLimit(
            price_limit, {"from": account})
...
def test_set_balances_to_spend():
    ...
    value = Web3.toWei(0.01, "ether")
    deposit_tx = staking_monitor.deposit(
        {"from": get_account(2), "value": value})
    deposit_tx.wait(1)
    ...
    assert (
        staking_monitor.s_userInfos(watch_list_entry_for_address)[
            "balanceToSpend"]
...
def test_can_set_price_limit():
    ...
    price_limit = 30000000000
    # Act
    price_bound_tx = staking_monitor.setPriceLimit(
        price_limit, {"from": account})
